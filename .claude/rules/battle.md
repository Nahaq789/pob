...
キー構造: `battle:{session_id}:static` / `battle:{session_id}:dynamic`

| 種別 | キー | 内容 |
|------|------|------|
| 固定 | `:static` | ベース実数値（nature補正済み）、タイプ、技リスト / バトル開始時のみ書き込み |
| 動的 | `:dynamic` | 現在HP、能力ランク（−6〜+6）、PP、状態異常、道具、特性 / 毎ターン更新 |
